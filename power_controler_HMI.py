...
class COMConnectorApp:
    def __init__(self, root):
        ...
        self.motor_command_frame.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
    ...
    def create_motor_command_buttons(self):
        self.start_button = tk.Button(self.motor_command_frame, text="Start", command=lambda: self.send_command(1))
        self.start_button.pack(pady=5)

        self.stop_button = tk.Button(self.motor_command_frame, text="Stop", command=lambda: self.send_command(2))
        self.stop_button.pack(pady=5)

        self.abort_button = tk.Button(self.motor_command_frame, text="Abort", command=lambda: self.send_command(3))
        self.abort_button.pack(pady=5)

        self.clear_button = tk.Button(self.motor_command_frame, text="Clear", command=lambda: self.send_command(4))
        self.clear_button.pack(pady=5)

        self.suspend_button = tk.Button(self.motor_command_frame, text="Suspend", command=lambda: self.send_command(5))
        self.suspend_button.pack(pady=5)

        self.unsuspend_button = tk.Button(self.motor_command_frame, text="Unsuspend", command=lambda: self.send_command(6))
